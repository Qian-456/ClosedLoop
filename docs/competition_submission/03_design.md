...
ClosedLoop 是一个执行型本地生活 Agent：用户一句话 → 生成 4–6 小时行程（玩→吃→后续）→ 用户确认 → Mock 执行预约/排队/库存扣减 → 关键失败进入补齐（fixup）继续把事情做完。  
...
- 执行侧：Mock 扣库存/扣容量是有副作用的，可能触发无座，无库存，必须支持失败兜底与低打扰补齐（fixup）以保证闭环成功率。
...
- 可复现工程化：执行前快照运行时数据（runtime JSON），一致性校验失败回滚并返回可重试错误码（工程证据详见 02_tools.md）。
...
说明：Main(8000) 负责对话路由与工具编排；Plan(8001) 负责确定性规划产出 itinerary + candidates；Search(8002) 为可选“先搜再换”；Mock DB 分静态读（规划期）与运行时写（执行期，含回滚）。
...
- 确认执行：`transfer_to_execute → execute_itinerary → mock_executor`；失败进入 `fixup_agent`，选候选/搜索后走 `adjust_and_execute_plan_item`。
...
- 工程证据（失败码/验收动作/实现索引/回滚一致性）统一收敛在 02_tools.md，design 只保留闭环叙事口径。

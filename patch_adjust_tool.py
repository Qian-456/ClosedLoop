...
with open('backend/src/closedloop/graph/tools/adjust_tool.py', 'r', encoding='utf-8') as f:
    content = f.read()

# Replace the tool import to also import `execute_tool`'s inner function, and add the new tool input class
imports_to_add = """
import time
...
"""
content = content.replace("class AdjustPlanItemInput(BaseModel):", imports_to_add + "\nclass AdjustPlanItemInput(BaseModel):")

# Find the body of adjust_plan_item to extract
match = re.search(r'(@tool\(args_schema=AdjustPlanItemInput\)\ndef adjust_plan_item\(.*?\) -> Command:\n.*?)', content, re.DOTALL)
if not match:
    print("Cannot find adjust_plan_item")
    exit(1)

old_tool = match.group(1)
...
"""
import typing
content = content.replace("from typing import Annotated, Literal", "from typing import Annotated, Literal, Any")
content = content.replace(old_tool, new_do_adjust)

with open('backend/src/closedloop/graph/tools/adjust_tool.py', 'w', encoding='utf-8') as f:
    f.write(content)

print("adjust_tool patched.")

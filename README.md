...
- `POST /invoke/stream`：输入一句话需求，返回 `text/event-stream`，按 LangGraph 状态快照持续推送 `state` / `done` / `error` 事件
- `POST /execute/start`：启动 Mock 执行，返回 execution\_id
...
前端开发时会通过 Vite proxy 转发 `/invoke`、`/invoke/stream`、`/health`、`/execute/*` 到后端（默认 `http://127.0.0.1:8000`）。

### `/invoke/stream` 事件契约

- `state`：中间态快照，`data.state` 为当前完整可序列化 Graph state
- `done`：最终态快照，`data.state` 为最终完整 state
- `error`：流式执行失败，`data.message` 为错误信息

说明：

- 当前消息流是“状态快照级 SSE”，不是逐 token 冒字。

...
在安装了后端依赖的 Python 环境、仓库根目录执行 `python -X utf8 scripts/acceptance_report.py`，打开输出路径中的 HTML 报告即可逐项查看支付门控、失败补齐与费用一致性回归，并下载同目录 JSON 和原始测试输出。详见 [验收步骤与面试证据](ACCEPTANCE.md)。报告使用固定模拟事件，不代表真实模型端到端、真实支付或生产性能验证。

...
- 触发已准备好的“无座 / 容量告急”场景
- 展示界面出现候选 1 / 候选 2
- 直接选择候选 1，触发**多层降级与权衡报告（Trade-off Report）**
...
大家可以注意到，此时系统不仅完成了替换，还主动用自然语言向我解释了**“为了完成这次替换，系统做出了哪些妥协”**，比如吃掉了 15 分钟的缓冲时间，或者把某个活动的体验时间极限压缩了。这是因为我们在底层实现了**多层冲突修复机制与权衡报告（Trade-off Report）**，让 Agent 的决策彻底透明。

这个步骤想证明的是，系统遇到失败时不是简单报错退出，而是能把用户继续带回可完成路径上，并且让用户对行程的调整“心中有数”。
...
- 异常覆盖与多层降级：是否在无座或容量不足时给出补救，并在替换超标时触发多层降级（吃 Buffer、压缩时长、删次要项），输出透明的权衡报告。
- 低打扰：是否尽量自己做决策，只有必要时才让用户选候选。
- 工程可信度：是否通过 Docker、服务拆分、Mock 支付与结构化材料证明系统是可复现、可解释的。
- 距离偏好默认值优化：当用户未提及或无特殊距离要求时，系统会默认使用 2km-5km，以兼顾候选质量和丰富度。
- 人数匹配：系统极其优先推荐精准匹配（如“2大1小”）的套餐。通过细粒度的等效权重计算（如未指定年龄儿童按 0.6 计算）和底层打分的硬过滤（Hard Filter），相差过大的套餐会被严厉惩罚甚至过滤，确保推荐结果的高关联度。

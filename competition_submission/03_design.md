...
- **多层降级与冲突修复**：当用户替换条目导致时间或预算超标时，底层触发多层压缩（吃 Buffer -> 极限压缩体验时间 -> 删减低优组件）。
- **权衡报告 (Trade-off Report)**：替换工具会将计算出的降级明细通过 `tradeoff_report` 回传，Agent 会在对话中向用户如实转达（例如：“为您替换了该活动，但吃掉了 15 分钟缓冲时间并压缩了下午茶”），让调整后果完全透明，而不是静默强行写入或简单报错。
